...
filepath = r"C:\Users\drcla\OneDrive\Senior Honours Project\Errors\Final_CLASSY_EMISSION_LINES_David.xlsx"
...
oiii_err = oiii_ratio * np.sqrt((o_iii_4363_err / o_iii_4363) ** 2 + (o_iii_5007_err / o_iii_5007) ** 2)
sii_err = sii_ratio * np.sqrt((s_ii_6717_err / s_ii_6717) ** 2 + (s_ii_6731_err / s_ii_6731) ** 2)
...
oii_err = oii_ratio * np.sqrt(first_term + second_term)
...
o_iii_ratio_err = o_iii_ratio * np.sqrt(((o_iii_4959_err**2 + o_iii_5007_err**2) / (o_iii_5007 + o_iii_4959)**2) + (h_beta_err/h_beta)**2)
o_ii_ratio_1_err = o_ii_ratio_1  * np.sqrt(((o_ii_3727_err**2 + o_ii_3729_err**2) / (o_ii_3729 + o_ii_3727)**2) + (h_beta_err/h_beta)**2)
o_ii_ratio_2_err = o_ii_ratio_2  * np.sqrt(((o_ii_7320_err**2 + o_ii_7330_err**2) / (o_ii_7320 + o_ii_7330)**2) + (h_beta_err/h_beta)**2)
n_ii_ratio_err = n_ii_ratio * np.sqrt((n_ii_6584_err/n_ii_6584)**2 + (h_beta_err/h_beta)**2)
...
oiii_T = np.zeros(len(oiii_ratio))
oiii_T_err_u = np.zeros(len(oiii_ratio))
oiii_T_err_d = np.zeros(len(oiii_ratio))

oii_T = np.zeros(len(oii_ratio))
oii_T_err_u = np.zeros(len(oii_ratio))
oii_T_err_d = np.zeros(len(oii_ratio))


oii_ab = np.zeros(len(oii_ratio))
oii_ab_err_u = np.zeros(len(oii_ratio))
oii_ab_err_d = np.zeros(len(oii_ratio))

oiii_ab = np.zeros(len(oii_ratio))
oiii_ab_err_u = np.zeros(len(oii_ratio))
oiii_ab_err_d = np.zeros(len(oii_ratio))

nii_ab = np.zeros(len(oii_ratio))
nii_ab_err_u = np.zeros(len(oii_ratio))
nii_ab_err_d = np.zeros(len(oii_ratio))

for i in range(len(o_iii_ratio)):
    
    ####Temperature errors

    #Get intensity distributions for each galaxy
    oiii_dist = np.random.normal(oiii_ratio[i], oiii_err[i], size = 300)
    oii_dist = np.random.normal(oii_ratio[i], oii_err[i], size = 300)
    sii_dist = np.random.normal(sii_ratio[i], sii_err[i], size = 300)

    #Distribution of densities and OIII region temperatures for galaxy
    [T_e_err,n_e_err] = diags.getCrossTemDen('[OIII] 4363/5007', '[SII] 6731/6716', oiii_dist, sii_dist)
    ...
    exp = (oii_T_e_err - 3000) / 0.7
    T_e_err = np.nan_to_num(T_e_err, nan = exp)
    #Replace T[OII] sample with Garnett
    oii_T_e_err = 0.7 * T_e_err + 3000
    
    #Take median of distributions to get T_e and n_e
    median_T_e = np.nanmedian(T_e_err)
    median_T_e_oii = np.nanmedian(oii_T_e_err)

    #Get error on median as values one sigma away from median
    #Upper error T
    T_e_err_up = np.nanpercentile(T_e_err, 84) - median_T_e
    #Lower error T
    T_e_err_down = median_T_e - np.nanpercentile(T_e_err, 16)
    #Upper error n
    ...
    print(f"The median OIII temperature for this galaxy ({i}) is {median_T_e}K")
    print(f"The median OII temperature for this galaxy ({i}) is {median_T_e_oii}K")

    oiii_T[i] = median_T_e
    oiii_T_err_u[i] = T_e_err_up
    oiii_T_err_d[i] = T_e_err_down
    oii_T[i] = median_T_e_oii
    oii_T_err_u[i] = T_e_oii_err_up
    oii_T_err_d[i] = T_e_oii_err_down

    print(f"{oiii_T} errors +{oiii_T_err_u}, -{oiii_T_err_d}")
    ...
    o_iii_dist = np.random.normal(o_iii_ratio[i], o_iii_ratio_err[i], size = 300)
    o_ii_dist_1 = np.random.normal(o_ii_ratio_1[i], o_ii_ratio_1_err[i], size = 300)
    o_ii_dist_2 = np.random.normal(o_ii_ratio_2[i], o_ii_ratio_2_err[i], size = 300)
    n_ii_dist = np.random.normal(n_ii_ratio[i], n_ii_ratio_err[i], size = 300)
    
    #Abundance distributions
    o_iii_ab = O3.getIonAbundance(o_iii_dist, tem = T_e_err, den = n_e_err, to_eval = "L(4959)+L(5007)", Hbeta = 1)
    ...
    #Upper errors
    oiii_err_up = np.nanpercentile(o_iii_ab, 84) - median_oiii_ab
    oii_err_up = np.nanpercentile(o_ii_ab, 84) - median_oii_ab
    nii_err_up = np.nanpercentile(n_ii_ab, 84) - median_nii_ab

    oiii_ab_err_u[i] = oiii_err_up
    oii_ab_err_u[i] = oii_err_up
    nii_ab_err_u[i] = nii_err_up

    #Lower errors
    oiii_err_down = median_oiii_ab - np.nanpercentile(o_iii_ab, 16)
    oii_err_down = median_oii_ab - np.nanpercentile(o_ii_ab, 16)
    nii_err_down = median_nii_ab - np.nanpercentile(n_ii_ab, 16)
    
    oiii_ab_err_d[i] = oiii_err_down
    oii_ab_err_d[i] = oii_err_down
    nii_ab_err_d[i] = nii_err_down

    print(f"Galaxy {i}/44 has metallicity {12 + np.log10(oiii_ab[i] + oii_ab[i])}")
    """
#Propagate errors
#Oxygen
o_ab = oiii_ab + oii_ab
...
#Metallicity
m_err_up = o_ab_err_up / (np.log(10)*o_ab)
m_err_down = o_ab_err_down / (np.log(10)*o_ab)
#Abundance ratio
ab_ratio = n_ab / o_ab
ab_ratio_err_up = ab_ratio * np.sqrt((n_ab_err_up/n_ab)**2 + (o_ab_err_up/o_ab)**2)
ab_ratio_err_down = ab_ratio * np.sqrt((n_ab_err_down/n_ab)**2 + (o_ab_err_down/o_ab)**2)

    
data = np.column_stack((m_err_up, m_err_down, ab_ratio_err_up, ab_ratio_err_down))
